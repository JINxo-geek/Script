...
def set_clipboard(url_list,fileName):
    for url in url_list:
        image = '!['+fileName+']' + '(' + url + ')' + '\n'
...
def window_main():
    if len(sys.argv) > 1:
        ...
        with open('markdownIMG.txt', 'a') as f:
            for url in url_list:
                image = '!['+fileName+']' + '(' + url + ')' + '\n'
                print 'Url:'+image, '\nFileName:'+fileFullName
                f.write(image)
        # print "\nThe ImgUrl saved in markdownIMG.txt"
        set_clipboard(url_list,fileName)
        sys.exit(-1)
        ...
        if added:
            ...
            with open('markdownIMG.txt', 'a') as f:
                for url in url_list:
                    ...
                    print 'URL:'+image, '\nFileName:'+fileFullName
                    f.write(image)
            # print "ImgUrl saved in markdownIMG.txt"
            set_clipboard(url_list,fileName)
        if removed:
            pass
        before = after
